...
def create_train_data():
    ...
    train_label_pairs = [(doc.tag, word_num) for doc in all_docs for word_num in doc.numbers]
...
def train(gpu_no, show_loss, train_data, label_data, word_embedding_size, doc_embedding_size,
    ...
    bert_doc_embeddings = utils.load('bert_doc_embeddings_100')
    ...
    with tf.name_scope('embeddings'):
        ...
        doc_embeddings = tf.Variable(bert_doc_embeddings)
    ...
    doc_embed = tf.nn.embedding_lookup(doc_embeddings, x_inputs[:, 0])
    final_embed = doc_embed
